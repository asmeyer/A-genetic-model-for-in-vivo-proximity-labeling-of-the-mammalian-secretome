...
# usage: MS data QC filtering script from the Carr lab modified by ASM to get QC data for combined BirA/Sox2Cre serum plexes. 
...
setwd("/Users/amandameyer/Box Sync/McMahon Lab/Dissertation/TR01/BirA-Sox2Cre/LC-MSMS/TR01_Sox2_LCMSMS/input/") 

# Set these parameters before running!
input_filename <- "TR01_USE_Plasma_AllDataTogether_Unfiltered_notNormalized_Multimedian_Correct.csv"
output_filename <- "TR01_USE_Plasma_AllDataTogether_Filtered_notNormalized_Multimedian_Correct.csv"
species_name <- "MOUSE"

# filter_at(vars(ends_with("TMT_126_total"):ends_with("TMT_131_total"))

input_file <- read.csv("TR01_USE_Plasma_AllDataTogether_Unfiltered_notNormalized_Multimedian_Correct.csv")
input_file_cols <- colnames(input_file)

#
#
protein_report <- read_delim(input_filename, delim = ",") # changed from ; to , 
...
filtered_PR <- protein_report %T>%
  ...
  filter_at(vars(starts_with("PlexesSearchedTogether_correct..126"):starts_with("PlexesSearchedTogether_correct..131")), all_vars(!is.na(.))) %T>% 
...
write_delim(filtered_PR, path = output_filename, delim = ",", na = "") # changed delim = ";" to ","
